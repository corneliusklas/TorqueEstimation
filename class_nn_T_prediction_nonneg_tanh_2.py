...
#    acceleration=False # die Beschleunigung wird als eingabeparameter verwendet
    ...
    savefilename="NN_T_predict_nonneg_tanh"    
...
#        print('std should be 1. This is how std looks: ',std)
    ...
    def build_model(self):
        ...
        model.add(keras.layers.Dense(16, input_shape=inputs,kernel_constraint=NonNeg,kernel_initializer=winit))
        model.add(keras.layers.Activation('tanh'))
        
        model.add(keras.layers.Dense(16,kernel_constraint=NonNeg,kernel_initializer=winit))
        model.add(keras.layers.Activation('tanh'))
    ...
    def plot_model(self):
        ...
        mpe_calc=np.mean(abs(np.divide(np.array(test_predictions_org)-np.array(test_labels_org),np.array(test_labels_org))))
